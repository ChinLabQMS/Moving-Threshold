...
startnumtot = validnewY(1)*scalingit;
...
% upgrade this part by fitting the scaling parameter- characterize the
% goodness of fit
...
% 28
% RingOccups2 and RingFill2 are the relevant arrays here
centerrange = 1:19;
centeroccups = RingOccups2(centerrange,:);
centeratoms = sum(centeroccups,1);
centersites = sum(ringPop(centerrange));
centerfilling = centeratoms/centersites;

sloperange = 23:30;
slopeoccups = RingOccups2(sloperange,:);
slopeatoms = sum(slopeoccups,1);
slopesites = sum(ringPop(sloperange));
slopefill = slopeatoms/slopesites;
...
% Choose nice, consistent colors
cmap = lines(2);  % 2-color map

% Plot data with markers and lines
plot(labels, centerfilling, '--o', ...
    'Color', cmap(1,:), ...
    'MarkerFaceColor', cmap(1,:), ...
    'LineWidth', 1.5, ...
    'DisplayName', 'Center (Rings 1–19)');

plot(labels, slopefill, '--s', ...
    'Color', cmap(2,:), ...
    'MarkerFaceColor', cmap(2,:), ...
    'LineWidth', 1.5, ...
    'DisplayName', 'Slope (Rings 23–30)');

% --- Axis labels and title ---
xlabel('Hold time (ms)', 'FontSize', 12, 'FontName', 'Helvetica');
ylabel('Filling fraction', 'FontSize', 12, 'FontName', 'Helvetica');

% --- Axes formatting for PRL ---
set(gca, ...
    'FontSize', 11, ...
    'FontName', 'Helvetica', ...
    'LineWidth', 1.2, ...
...
% --- Legend ---
legend('show', 'Location', 'northeast', 'FontSize', 10, 'Box', 'off');
%%
% --- Optional: Export as PDF ---
print(gcf, 'filling_vs_time_center_vs_slope', '-dpdf', '-r300');

figure;
plot(labels,centerfilling,'o');
hold on
plot(labels,slopefill,'o');
